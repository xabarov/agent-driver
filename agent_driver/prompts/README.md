...
- `python_tool_system_addendum.txt`: dynamic python tool sandbox/imports block (must include `{imports}`, `{policy_summary}`, `{max_exec_ms}`, `{max_output_chars}`, and `{session_idle_seconds}` placeholders).
