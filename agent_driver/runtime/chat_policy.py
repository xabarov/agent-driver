...
def is_python_reliability_request(message: str) -> bool:
    """Return whether chat policy should force Python from text alone.

    Kept as a compatibility seam for older callers, but intentionally returns
    False. Python usage is now prompt-guided and scenario-tested explicitly
    instead of inferred from brittle text/URL heuristics.
    """
    _ = message
    return False
