...
def test_curated_research_skill_metadata_uses_registered_tool_names() -> None:
    manifests, _ = list_skill_manifests(
        base_dir=curated_skills_dir(),
        trusted_roots=(curated_skills_dir(),),
    )
    ...
    for manifest in manifests:
        unknown = set(manifest.allowed_tools) - KNOWN_RESEARCH_TOOLS
        assert not unknown, f"{manifest.name} references unknown tools: {unknown}"
