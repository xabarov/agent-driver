...
| **D4** ✅ | **Harness profile layer** (DONE 2026-06-09) — `HarnessProfile` contract (`contracts/profiles.py`) + pure `harness/` helpers, wired into request assembly (`build.py`) via `RunnerConfig(harness_profiles=...)`. Per-model first-match selection (`match_models` fnmatch globs; empty = any) shapes the request: `system_prefix`/`system_suffix` slots wrap the assembled system prompt **before trimming**; `excluded_tools` ride the deny filter (model never sees them); `tool_description_overrides` rewrite surfaced schemas. Matching uses the resolved (forced) model — per-model profiles need a pinned model, empty-pattern = provider default. Tests: helper unit tests (select/slots/exclude/override/no-mutation) + end-to-end through the runner (suffix + exclusion + override reach the provider). (Per-model *middleware* deferred — lifecycle hooks already compose; the high-value declarative wins were prompt+tools) | a declarative layer over descriptor providers (#8) + prompt templates | Med · Med |
