...
|---|---|---|---|
| U1 | 049 embedding-facade | **Phase A + C DONE** — facade-довод (store/command/hook/stream) + exact export-снапшот (sdk/runtime/tools) + deprecation-policy в embedding.md; `__version__` в U7; свип 2968. **A+C+D+E DONE** (facade-довод + export-снапшот + e2e-пример + единый `agent_driver.embedding`-namespace). **U1 ЗАКРЫТ.** | средне |
