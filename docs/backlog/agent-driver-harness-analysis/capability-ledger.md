...
| U3 | 051 atomic-approval | **контракт-срез + B/C/D-ядро DONE** — `ApprovalConsumptionStore` (in-mem+SQLite CAS-ledger, facade-экспорт, опц. `RunnerConfig(approval_store=)`): exactly-once consume, доказан один side-effect под 16-поточной гонкой + 2 concurrent resume + restart; свип 2919. Осталось: prior-result full-replay, монотонный checkpoint-revision | **крупнейшее** |
| U4 | 052 durable-stop | **C(hook) + A/D-ядро + B(границы) DONE** — `AbortLifecycleStore` (in-mem+SQLite `requested→observed→cancelled/completed_before_cancel`, выставляет `observed`, restart-queryable, facade+`RunnerConfig(abort_store=)`); `_finalize_abort_lifecycle` в раннере; свип 2933. Осталось: mid-LLM-await abort, fencing-token, cancellation-failed/late-ignored terminal, deadline | крупно |
| U5 | 053 plan-integrity | **ядро A/B/C + enforcement DONE** — harness-авторский hash + EDIT-re-hash + `detect_plan_revision` + host policy-binding + `PlanningPolicyInput.required_plan_hash` (гейт сравнивает hash → DENY на ревизии); свип 2939. Осталось: подключить durable `PlanArtifactStore` + binding через trace | средне |
