...
|---|---|---|---|
| U1 | 049 embedding-facade | **Phase A + C DONE** — facade-довод (store/command/hook/stream) + exact export-снапшот (sdk/runtime/tools) + deprecation-policy в embedding.md; `__version__` в U7; свип 2965. **Phase D DONE** (e2e-пример `19_embedded_e2e.py` + cookbook off internal-путей). Осталось: единый `embedding`-namespace (опц., косметика) | средне |
| U2 | 050 gate-identity+provenance | **A/B/D + interrupt_id-унификация DONE** (identity + `GateProvenance` под `_ad_` + fail-closed валидация + ask-path→interrupt; **общий `build_interrupt_id` → `int_{run_id}_{tool_call_id or index}`**; свип 2964); осталось C/E (terminal-провенанс-проекция, retry/timeout/abort-матрица) | крупно |
| U3 | 051 atomic-approval | **контракт-срез + B/C/D-ядро DONE** — `ApprovalConsumptionStore` (in-mem+SQLite CAS-ledger, facade-экспорт, опц. `RunnerConfig(approval_store=)`): exactly-once consume, доказан один side-effect под 16-поточной гонкой + 2 concurrent resume + restart; **+ F2 prior-result replay** (`result_payload` в ledger + `RunnerConfig(replay_prior_result=)` → дубль возвращает прежний output verbatim; свип 2957) **+ F3 монотонный checkpoint-revision** (`CheckpointRef.revision` + `ResumeCommand.expected_revision`-guard, Вариант A additive; свип 2960). Осталось: Вариант B `latest()`-by-revision-rewire (отложен, низкая марж.) | **крупнейшее** |
