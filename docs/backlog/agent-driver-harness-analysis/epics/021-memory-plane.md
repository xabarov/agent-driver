...
## Наблюдение 2026-07-19 (после рефакторинг-приёмки): extraction-JSON флейк

deepseek периодически отвечает прозой вместо JSON-массива → fail-open в raw-turn
(без slot) → одноимённые факты из разных сессий не схлопываются supersede'ом →
при накоплении recall может отдать УСТАРЕВШИЙ вариант факта. На чистом сторе
смоук стабильно PASS. Кандидаты фикса (фаза B+): bounded-retry extraction при
непарсибельности; structured-output режим провайдера; recall-инструкция «при
противоречии верить новейшей записи» (уже частично работает — supersede-сценарий
PASS даже при загрязнении).
