...
`powershell_tool` дает policy-compatible sibling для shell на базе `pwsh`.
...
`agent_tool` создает структурированный payload для запроса на запуск subagent:
...
`send_message_tool` создает session-local message event для teammate/subagent
...
`list_peers_tool` возвращает session-local directory peers с фильтрами по status
...
`team_create_tool` и `team_delete_tool` управляют session-local team registry:
...
`team_get_tool` и `team_list_tool` дают read-only доступ к этому team registry:
...
`enter_worktree_tool` и `exit_worktree_tool` формируют high-risk request
...
`cron_list_tool`), `remote_trigger_tool`, `subscribe_pr_tool`,
`push_notification_tool`, `send_user_file_tool` пока работают как local intent
