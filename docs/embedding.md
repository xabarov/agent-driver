...
| Import | What you get |
| --- | --- |
| `agent_driver.embedding` | **One aggregate namespace** re-exporting the embedding-essentials from the facades below (identity re-exports — no new API, nothing to drift): `create_agent`/`Agent`/`Session`, `RunnerConfig`/`runner_config_parameter_names`/`RunAbortHandle`, host-store protocols + durable impls (`SqliteRuntimeStore`, `SqliteApprovalConsumptionStore`, `SqliteAbortLifecycleStore`, …), `RunLifecycleHook`/`BaseRunLifecycleHook`, `ToolGate*`/`GateProvenance`, provider protocol + `FakeProvider`, `register_skill_tools`, `AgentRunInput`/`AgentRunOutput`/`ResumeCommand`. Use this for a durable embedding from a single import root; use the per-concern facades below for the full surface. |
