...
лучше потому, что дошел до синтеза по открытым источникам, а наш ответ честно
...
- [x] Slice 5: проверить, не нужен ли маленький runtime guard после успешного
  ...
  2026-05-31 update: text/regex inference for `python_reliability_request` was
  removed after it produced false positives on URLs and research prompts.
  Python remains prompt-guided and explicitly asserted in scenarios that require
  it; runtime guards still work when metadata is set directly.
...
- [x] Добавить scenario verdict labels:
  ...
  2026-05-31 update: `missed_python` and `unnecessary_python` are no longer
  emitted from prompt heuristics; Python correctness is checked by explicit
