...
| `open_todos_finalize_blocked` | transient | модель попыталась дать финальный ответ, но в плане сессии остались открытые todos (planning P1); переспрашиваем «доделай или отмени» вместо финализации; bounded ≤3, затем всё равно финализируем | «Завершаю запланированные шаги…» (диагностика) |
| `plan_verification_nudge` | transient | модель попыталась финализировать завершённый план из 3+ шагов без шага верификации (planning P5); один раз переспрашиваем «сначала проверь работу»; bounded ≤1 | «Проверяю результат перед ответом…» (диагностика) |
| `planned_tool_call_step_limit_applied` | transient (severity=info) | провайдер вернул больше tool calls, чем разрешено для одного шага; допустимый префикс продолжает работу, остальные вызовы подавлены до нового решения модели | не для статус-строки; в диагностике показать `accepted_count` / `suppressed_count` |
| `planned_tool_call_budget_limit_applied` | transient (severity=info) | parallel batch превысил остаток общего `max_tool_calls`; допустимый префикс исполнен, лишние вызовы подавлены до любого approval/contact | не для статус-строки; в диагностике показать `remaining_before_step`, `accepted_count` / `suppressed_count` |
...
| `control_kind_unsupported` | transient | стиринг-команда `control_kind` неизвестна этой сборке (version skew, эпик 030); помечена FAILED, не молчаливый drop | «Команда не поддержана» (диагностика очереди) |
| `control_kind_not_implemented` | transient | стиринг-команда распознана, но не имеет потребителя в этом контексте (напр. `STOP_SUBAGENT`/`CONTINUE_SUBAGENT` на single-agent диспетчере, A6); помечена FAILED | «Команда пока не реализована» (диагностика очереди) |
