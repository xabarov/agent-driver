...
| `approved_plan`, `approved_prompts`, `clarification`, `last_todo_write_signature`, `todo_write_deduped` | planning tools, approval flow, output | output/checkpoint | yes for planning UI | `PlanningRuntimeState`; `approved_prompts` preserves the exact runtime-proposed categories accepted by the host across resume/checkpoint handoff |
