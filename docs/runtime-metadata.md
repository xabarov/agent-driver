...
| `execution_lease_ref`, `execution_lease_failure` | EPIC-03 execution-lease manager (runner) — the SAFE, non-secret, generation-bound `ExecutionLeaseRef` (JSON) re-attached on resume, and a redaction-safe fail-closed reason when a requested lease could not be secured | checkpoint | diagnostics | `agent_driver.execution` lease manager |
