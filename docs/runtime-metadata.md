...
| `rolling_summary`, `rolling_summary_covers_upto` | Option B2 amortized rolling summary: the persisted cumulative compaction summary text and the source-group cursor it covers; each llm_full firing folds the prior summary + only the groups past the cursor (opt-in via `enable_rolling_summary`, default off) | output/checkpoint | compaction amortization | `_apply_llm_full_compaction` (rolling branch) |
