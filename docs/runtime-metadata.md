...
| `resume_action`, `resume_message`, `resume_target_step`, `pending_interrupt`, `interrupt_payload`, `consumed_approvals`, `rejection_feedback` | resume/interrupt flow, output builder | checkpoint/output | yes for resume/interrupt UI | `LoopControlState` plus interrupt contract; `consumed_approvals` records consumed interrupt/idempotency ids for U3 idempotent-replay detection; `rejection_feedback` (EPIC-04, opt-in) carries an operator's correcting-rejection `{tool_name, feedback}` and is popped by the LLM-request builder after one-shot injection as a steering turn |
...
| `open_todos_finalize_reprompt_count`, `open_todos_finalize_blocked` | tool stage finalize gate (planning P1) | checkpoint/transient | diagnostics | bounded (≤3) re-prompt when a run tries to finalize with pending/in_progress todos; `open_todos_finalize_blocked` is a one-shot marker consumed by the next request to inject the "finish or cancel" reminder |
| `verify_before_final_reprompt_count`, `verify_before_final_blocked` | tool stage finalize gate (planning P5) | checkpoint/transient | diagnostics | bounded (≤1) re-prompt when a run tries to finalize a completed 3+ step plan with no verification step; `verify_before_final_blocked` is a one-shot marker consumed by the next request to inject the "verify your work first" reminder |
...
| `effective_tool_names`, `tool_choice_override`, `force_final_answer`, `force_final_answer_reason`, `forced_tool_choice_retry`, `forced_tool_catalog`, `repeat_call_guard_threshold` | llm/tool-call preparation and repair; `repeat_call_guard_threshold` is the seeded-from-`RunnerConfig` doom-loop threshold (EPIC-02) read by `_has_repeated_recent_tool_call` | checkpoint | diagnostics | `ToolLoopState` or `ProviderRuntimeState` |
...
| `approved_plan`, `approved_prompts`, `clarification`, `plan_refinement_required`, `plan_refinement_revision_count`, `plan_mode_exit_revision_count`, `last_todo_write_signature`, `todo_write_deduped` | planning tools, approval flow, output | output/checkpoint | yes for planning UI | `PlanningRuntimeState`; `approved_prompts` preserves the exact runtime-proposed categories accepted by the host across resume/checkpoint handoff; plan-refinement state keeps a clarified approval plan non-terminal until a revised artifact is produced or the bounded retry fails closed; `plan_mode_exit_revision_count` bounds forced recovery when a model tries to finish active plan mode as prose |
...
| `raw_assistant_content`, `last_llm_response`, `llm_call_started_monotonic`, `llm_routed_role` | LLM step/output builder; `llm_routed_role` = the model_role an async `LlmDifficultyRouter` (R8) resolved once per run, reused by the sync build path every turn | checkpoint/runtime | diagnostics | `StreamingRuntimeState` or `ProviderRuntimeState` |
...
| `live_tool_prune` | EPIC-08 pre-pass audit: cleared count + chars saved + keep_recent + pressure state when the live `ToolResultPruner` clears OLD tool-result content in the ephemeral request (runs independently of `enable_compaction`, pressure-gated, commit-threshold) | checkpoint | trace/debug | compaction stage (`clear_old_tool_results`) |
...
| `parent_run_id`, `subagent_group_id`, `subagent_depth` | subagent child-run handoff identity (set by the subagent executor, read by the runner for the EPIC-03 default ISOLATE lease policy — a child run neither acquires nor releases the parent's execution lease). `subagent_depth` is the child's depth in the subagent tree (top-level = 0), enforced against `RunnerConfig.max_subagent_depth` for governed recursion (C7). | checkpoint | diagnostics | `SubagentRuntimeState` |
...
| `last_provider_error`, `last_provider_diagnostics`, `last_provider_stream_error`, `max_tokens_retry`, `provider_max_tokens_source`, `empty_forced_final_retry`, `forced_final_retry`, `forced_final_empty_after_all_retries`, `forced_final_prior_turn_recovered`, `forced_final_fallback_provider`, `poisoned_prefix_quarantine_attempted`, `poisoned_prefix_suspect_turns`, `poisoned_prefix_quarantine_recovered`, `wall_clock_guard`, `tool_failure_guard`, `tool_failure_nudge_due`, `tool_failure_nudge_sent`, `refunded_tool_calls`, `context_window_resolved`, `context_window_unresolved_warned`, `reasoning_echo_retry`, `transient_provider_retries`, `transient_transport_retries`, `model_fallbacks`, `prompt_cache_state`, `context_overflow_recovery`, `context_overflow_emergency_strip`, `provider_stream_non_stream_fallback`, `provider_stream_fallback_diagnostics` | provider retry/recovery (incl. reactive compact-and-retry on context overflow, the EPIC-10 emergency payload strip audit `{cleared, truncated, chars_saved, …}` on the overflow retry, and streaming fallback diagnostics) | checkpoint | provider diagnostics | `ProviderRuntimeState` |
...
| `steering_pause_requested` | transient marker set by the control dispatcher when a steering `PAUSE` control is drained; the LLM step consumes it at the next boundary to park the run as a resumable `MANUAL_PAUSE` interrupt (A3) | transient | live-control diagnostics | `_apply_control_item` (PAUSE branch) / `_park_steering_pause` |
...
| `llm_generation`, `live_message_terminal_reconciliation`, `live_message_terminal_reconciliation_error` | live-message generation fence and raw-free terminal promotion/Stop reconciliation; the error key records only the reconciliation exception class when fail-open cleanup cannot commit and never replaces the provider failure | checkpoint/output | live-control diagnostics | versioned live-message control state |
...
| `memory_pending_writes`, `memory_explicit_write_count`, `memory_explicit_synced_count` | model-authored `remember` writes buffered during the run, count buffered, and count flushed to the store at completion (epic M1); a non-empty buffer makes the hook skip the automatic turn-sync/extraction for that turn | checkpoint | diagnostics | `apply_memory_writes_from_envelopes` (tool stage) / `MemoryLifecycleHook.on_run_completed` |
...
| `skills_catalog_block` | Skills S1 tier-1 "available skills" catalog (name + one-line summary + base_dir), scanned from `skills_catalog_sources` once per run and injected into the system prompt when a skill-load tool is available; re-built each request so it survives compaction | checkpoint | diagnostics | `_skills_catalog_block` (prompt build) / `build_skills_catalog_block` |
| `skills_keyword_hints_block` | Skills S5 keyword-trigger hints — skills whose declared `keywords` match the run input, surfaced with a load hint; same sources/gate as the catalog, cached once per run (input is stable) and re-injected into the system prompt each request | checkpoint | diagnostics | `_skills_keyword_block` (prompt build) / `build_skill_keyword_hints` |
...
| `context_chars_per_token` | per-run calibrated chars-per-token ratio (BUG-6): updated from the provider's actual input-token count each turn and fed into the next pre-send pressure/budget estimate; clamped to a sane range | output/checkpoint | budget/pressure accuracy | `CompactionRuntimeState` |
...
| `token_pressure.occupancy_pct` + `llm_call_completed.context_occupancy_pct` | occupancy telemetry (horizon-scan 047): the fraction of the compaction trigger the post-trim prompt filled, from the cached threshold (no probe); carried on every `llm_call_completed` so a trace can tell a dormant compaction plane (occupancy stays low, never fires) from a firing one. The run-trace `context_pressure` summary aggregates it into `max_occupancy_pct` + `compaction_plane_dormant` | output/event | context-pressure diagnostics | `estimate_token_pressure` → `_build_llm_completed_payload` → `context_pressure_summary` |
| `rolling_summary`, `rolling_summary_covers_upto`, `rolling_skip_count`, `rolling_cadence_deferred` | Option B2 amortized rolling summary: the persisted cumulative compaction summary text + the source-group cursor it covers; each llm_full firing folds the prior summary + only the groups past the cursor (opt-in via `enable_rolling_summary`, default off). `rolling_summary_every_n_turns` cadence defers the fold on non-cadence firings (`rolling_skip_count` counter, `rolling_cadence_deferred` marker) so the prompt-cache prefix is rewritten every N firings instead of every one; never defers under blocking pressure | output/checkpoint | compaction amortization | `_apply_llm_full_compaction` + `_rolling_cadence_should_defer` |
