...
| `tool_results`, `tool_trace`, `tool_calls`, `tool_loop_iterations`, `max_tool_calls`, `empty_answer_retry_count`, `degenerate_answer_retry_count` | tool stage, output, research contract | output/checkpoint | yes, trace/debug | `ToolLoopState` (`empty_answer_retry_count`: epic 015 bounded empty-answer re-prompt; `degenerate_answer_retry_count`: epic 015 bounded canned/wrong-language refusal re-prompt) |
...
| `last_provider_error`, `last_provider_diagnostics`, `last_provider_stream_error`, `max_tokens_retry`, `empty_forced_final_retry`, `forced_final_retry`, `forced_final_empty_after_all_retries`, `forced_final_prior_turn_recovered`, `forced_final_fallback_provider`, `wall_clock_guard`, `tool_failure_guard`, `refunded_tool_calls`, `context_window_resolved`, `reasoning_echo_retry`, `transient_provider_retries`, `context_overflow_recovery`, `provider_stream_non_stream_fallback`, `provider_stream_fallback_diagnostics` | provider retry/recovery (incl. reactive compact-and-retry on context overflow and streaming fallback diagnostics) | checkpoint | provider diagnostics | `ProviderRuntimeState` |
