...
| `last_provider_error`, `last_provider_diagnostics`, `last_provider_stream_error`, `max_tokens_retry`, `empty_forced_final_retry`, `forced_final_retry`, `forced_final_empty_after_all_retries`, `reasoning_echo_retry`, `context_overflow_recovery`, `provider_stream_non_stream_fallback`, `provider_stream_fallback_diagnostics` | provider retry/recovery (incl. reactive compact-and-retry on context overflow and streaming fallback diagnostics) | checkpoint | provider diagnostics | `ProviderRuntimeState` |
