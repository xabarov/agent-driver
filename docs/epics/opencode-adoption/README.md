...
| 02 | S | **Doom-loop / repetition detector** | Ring of last-N `(tool_name, canonical_args_hash)`; 3 identical consecutive → force the existing budget-grace synthesis turn (or a HITL redirect), not a silent step cap. From opencode `processor.ts` doom-loop gate. |
| 03 | S | **Decision-hook seam on the gate chain** | Ordered host-registered `PolicyHook`s around `evaluate_tool_policy`/`ToolGate` that can *tighten* (never loosen past a hard DENY) allow/deny/ask and rewrite a tool's model-facing description/params. From opencode `permission.ask` + `tool.definition` plugin hooks. Lets excel-ai/Zion inject governance without forking. |
