...
| 11 | L | **Durable, resumable subagent identity** 🟡 | **Stage 1 + the Stage-2 Postgres substrate DONE → [`EPIC-11`](EPIC-11-durable-subagent-identity.md).** `SubagentStore.find_run_by_child_run_id` (InMemory + Sqlite JSON1 + now Postgres) + `Agent.find_subagent_run(child_run_id)` make a child addressable by its own id; `PostgresSubagentStore` puts durable run/group state on the shared PG control plane (idempotency + indexed `child_run_id`, live-verified vs `postgres:16`). **Deferred (the risky rest of the L):** `resume_subagent(child_run_id, prompt)` checkpoint-replay execution (children share the parent thread + run under a per-spawn id → touches the child-execution/checkpoint contract) and fg→bg `promote` / running-child `extend`. |
