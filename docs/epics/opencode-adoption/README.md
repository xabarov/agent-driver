...
| 06 | M | **Real outward MCP client** 🟡 | **stdio + streamable-HTTP DONE → [`EPIC-06`](EPIC-06-mcp-client.md), both live-verified against `@modelcontextprotocol/server-everything`.** `agent_driver/tools/mcp_client/`: `StdioMcpClient` (dependency-free JSON-RPC over subprocess stdio) + `HttpMcpClient` (streamable-HTTP over `httpx` — SSE parsing + `Mcp-Session-Id` handling; no new dep), a shared transport-agnostic `register_mcp_client` core, and `register_stdio_mcp_server` / `register_http_mcp_server` that discover tools and register them governed + namespaced (`mcp__<server>__<tool>`, EXTERNAL_ACTION/ON_POLICY_MATCH, allowlist). SDK server-list wiring via `sdk.connect_mcp_servers(agent, configs)` / `close_mcp_servers`. **Deferred:** OAuth2+PKCE (bearer/header already works via `HttpServerConfig.headers`), ACP `mcp_servers` param wiring, `tools/list_changed` refresh. The `tools/builtin/mcp.py` fixtures stub stays for back-compat. |
